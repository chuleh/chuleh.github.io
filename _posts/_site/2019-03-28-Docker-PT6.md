Hi, in today's lecture we will continue with our `Dockerfile` tutorial. In our [previous lecture]({% post_url 2019-03-25-Docker-PT5 %}) we created a `Dockerfile` for our `redis-server` and saw how to `build` and `run` it.
